...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 font-sans selection:bg-[#C2410C] selection:text-white">
      
      {/* --- TOP CONTACT BAR --- */}
      <div className="bg-zinc-950 text-zinc-300 py-2 px-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-xs font-medium tracking-wide">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 text-[#C2410C]" /> +91 9246546742</span>
            <span className="hidden sm:flex items-center gap-2"><Mail className="w-3.5 h-3.5 text-[#C2410C]" /> accounts@jyanipur.in</span>
          </div>
          <div className="flex items-center gap-2">
            <MapPin className="w-3.5 h-3.5 text-[#C2410C]" /> Hyderabad, Telangana
          </div>
        </div>
      </div>

      {/* --- HEADER --- */}
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-lg border-b border-zinc-200 px-6 py-4 transition-all">
      ...
      <section className="relative pt-12 pb-20 lg:pt-24 lg:pb-32 px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8 z-10">
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-900 bg-zinc-100 px-4 py-2 rounded-full border border-zinc-200">
          ...
          <div className="relative z-10 lg:ml-auto w-full max-w-md">
            <div className="absolute inset-0 bg-zinc-100 blur-3xl -z-10 rounded-full"></div>
            <div className="bg-white/80 backdrop-blur-xl border border-zinc-200 p-8 rounded-3xl shadow-2xl">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full h-2 bg-zinc-200 rounded-full appearance-none cursor-pointer accent-[#C2410C]"
                  />
                </div>

                <div className="bg-zinc-950 text-white p-6 rounded-2xl">
                  <span className="text-xs text-zinc-400 uppercase tracking-widest font-bold block mb-1">Estimated Budget</span>
                  ...
                  <button 
                    ...
                    className="w-full bg-[#C2410C] hover:bg-[#9A3412] text-white font-bold py-3 rounded-xl transition-colors text-sm"
                  >
      ...
      <section id="about" className="py-24 bg-zinc-50 px-6 border-t border-zinc-100">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {capabilities.map((cap, i) => (
              <div key={i} className="bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-zinc-200">
      ...
      <section id="portfolio" className="py-24 px-6 max-w-7xl mx-auto">
            ...
            {['All', 'Residential', 'Civil', 'Interiors'].map(filter => (
              <button
                ...
                className={`px-5 py-2.5 text-sm font-bold rounded-full transition-all border ${
                  activeFilter === filter ? 'bg-zinc-900 text-white border-zinc-900 shadow-md' : 'bg-white text-zinc-600 border-zinc-200 hover:bg-zinc-50'
                }`}
              >
                {filter}
              </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {filteredProjects.map((p) => (
            <div 
              key={p.id} 
              className="group bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer"
            ...
            >
              ...
              <div className="p-8">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="bg-zinc-100 border border-zinc-200 text-zinc-900 text-xs font-bold px-3 py-1 rounded-full">
                    {p.area}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-center items-center p-4 bg-zinc-950/80 backdrop-blur-sm">
          <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-3xl overflow-hidden flex flex-col relative shadow-2xl">
            <button 
              ...
              className="absolute top-4 right-4 z-10 bg-white/50 hover:bg-white text-zinc-900 p-2 rounded-full backdrop-blur-md transition-colors shadow-sm"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <div className="p-8 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                {selectedProject.features.map((feat, i) => (
                  <div key={i} className="flex items-center gap-3 bg-zinc-50 p-4 rounded-xl border border-zinc-200">
              ...
              <div className="bg-zinc-950 p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  <h4 className="font-bold text-white">Want to build something similar?</h4>
                  <p className="text-sm text-zinc-400 mt-1">Get a transparent, detailed BOQ for your plot today.</p>
                </div>
                <button 
                  onClick={() => {
                  ...
                  className="w-full sm:w-auto bg-[#C2410C] hover:bg-[#9A3412] text-white px-6 py-3 rounded-full text-sm font-bold transition-all whitespace-nowrap"
                >
                  Get Free Estimate
                </button>
              </div>
          ...
          <div className="bg-white w-full max-w-md p-8 rounded-3xl relative shadow-2xl">
            ...
            {!isLeadSaved ? (
              <>
                ...
                <form onSubmit={handleEstimateSubmit} className="space-y-5">
                  <div>
                    ...
                    <input type="text" required value={leadData.clientName} onChange={e => setLeadData({...leadData, clientName: e.target.value})} placeholder="e.g. Ramesh Varma" className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 text-zinc-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#C2410C] transition-all" />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-zinc-700 uppercase mb-2 ml-1">Phone</label>
                      <input type="tel" required value={leadData.phone} onChange={e => setLeadData({...leadData, phone: e.target.value})} placeholder="+91 98765..." className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 text-zinc-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#C2410C] transition-all" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-zinc-700 uppercase mb-2 ml-1">Area (Sq.Ft)</label>
                      <input type="number" required value={leadData.sqft} onChange={e => setLeadData({...leadData, sqft: e.target.value})} placeholder="2500" className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 text-zinc-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#C2410C] transition-all" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-zinc-700 uppercase mb-2 ml-1">Project Type</label>
                    <select value={leadData.projectType} onChange={e => setLeadData({...leadData, projectType: e.target.value})} className="w-full px-5 py-4 bg-zinc-50 border border-zinc-200 text-zinc-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#C2410C] transition-all cursor-pointer">
                  ...
                  <button type="submit" disabled={loading} className="w-full py-4 bg-[#C2410C] hover:bg-[#9A3412] text-white font-bold rounded-full transition-all mt-4 shadow-lg hover:shadow-xl hover:-translate-y-0.5">
              ...
              <div className="py-10 text-center space-y-4">
                <div className="w-16 h-16 bg-zinc-100 text-[#C2410C] flex items-center justify-center mx-auto rounded-full mb-6">
      ...
      {/* --- CTA SECTION --- */}
      <section className="py-24 bg-zinc-50 px-6 text-center border-t border-zinc-200">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-black text-zinc-900 tracking-tight mb-6">Ready to break ground?</h2>
          <p className="text-zinc-600 text-lg mb-10">Stop worrying about hidden costs and delayed timelines. Let Jyanipur handle everything from architectural blueprints to the final coat of paint.</p>
          <button 
            onClick={() => setIsEstimateModalOpen(true)}
            className="bg-[#C2410C] hover:bg-[#9A3412] text-white font-bold px-10 py-5 rounded-full text-lg shadow-xl hover:scale-105 transition-transform"
          >
